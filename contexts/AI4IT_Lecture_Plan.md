...
**Environment setup, not just the webpages below:** **Day 3** (not Day 4) now requires each participant to install Langflow locally, since the Langflow build moved into Day 3's second half — the same "make it personal" pattern as Day 1's Antigravity install and Day 2's Ollama install. This is real, self-hosted software the room runs on their own machines, not a custom page this codebase builds. **It must be installed and verified before the weekend, not live in the room** — a ~50-person parallel first-time install is the single biggest practical failure risk in Weekend 2. See `day3_prompt_engineering_spec.md` §0.1 for the full pre-session checklist (install verification, API keys, fallback pairing, importable rescue flow, checkpoint gates).
